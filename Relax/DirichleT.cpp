...
//  Допилить метод для записи в файл
void DirichleT::OutputFile(int _sizeX, int _sizeY, TVector<double> _borderX, TVector <double> _borderY) {
	
	//  Заполенине файла
	ofstream fout;
	fout.open("data.txt");

	//  Записать размерности
	fout << _sizeX + 1 << endl;
	fout << _sizeY + 1 << endl;

	//  Записать границы
	//  по Х
	fout << _borderX[0] << endl;
	fout << _borderX[1] << endl;
	//  Шаг X
	fout << h << endl;

	//  по Y
	fout << _borderY[0] << endl;
	fout << _borderY[1] << endl;
	//  Шаг Y
	fout << k << endl;
